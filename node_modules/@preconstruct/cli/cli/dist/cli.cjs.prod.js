...
class Entrypoint extends Item {
  constructor(filePath, contents, pkg) {
    super(filePath, contents), _defineProperty$2(this, "package", void 0), _defineProperty$2(this, "_sourceCached", void 0), 
    _defineProperty$2(this, "_strict", void 0), this.package = pkg;
  }
  get name() {
  ...
  get source() {
    return void 0 === this._sourceCached && (this._sourceCached = resolve.sync(path.join(this.directory, this.configSource), {
  ...
  updater(json) {
    super.updater(json), this._sourceCached = void 0;
  }
}
  ...
  static async create(directory, project) {
    ...
    if (pkg.project = project, project.experimentalFlags.newEntrypoints) {
      ...
      pkg.entrypoints = await Promise.all(entrypoints.map(async sourceFile => {
        let directory = path.join(pkg.directory, path.resolve(sourceFile).replace(path.join(pkg.directory, "src"), "").replace(/\.[tj]sx?$/, ""));
        "index" === path.basename(directory) && (directory = path.dirname(directory));
        ...
        return Promise.all(descriptors.map(async ({filename: filename, directory: directory, contents: contents, hasAccepted: hasAccepted}) => {
...
async function writeDevTSFile(entrypoint, entrypointSourceContent) {
  let hasDefaultExport = await entrypointHasDefaultExport(entrypoint, entrypointSourceContent), cjsDistPath = path.join(entrypoint.directory, entrypoint.main).replace(/\.js$/, "");
